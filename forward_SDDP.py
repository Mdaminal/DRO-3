...
def forward(stage,res,stages,cen,g,simu,cuts,o):
    ...
    if bacia==1:
        ...
        for h in hidros.index:
            ...
            for q in range(ordem):
                # stage[j]['yp'][h,q].UB = y_hist.loc[hidros.COMPATIBILIDADE_SPT.loc[h], y_hist.columns == t-relativedelta(months=+q+1)][0]
                # stage[j]['yp'][h,q].LB = y_hist.loc[hidros.COMPATIBILIDADE_SPT.loc[h], y_hist.columns == t-relativedelta(months=+q+1)][0]
    ...
    # stage[j]['m{0}'.format(j)].write('SDDP_11.lp')
    # stage[j]['m{0}'.format(j)].write('SDDP_11.sol')
    ...
    for s in cen:
        ...
        if bacia==1:
            ...
            for h in hidros.index:
                # cen[s][f'ysult{h}'] = [stage[1]['yn'][h].x]
    ...
    for s in cen:
        ...
        for j in range(2,stages):
            ...
            if bacia==1:
                for b in hidros.BACIA.unique(): 
                    for q in range(ordem):
                        # stage[j]['yp'][b,q].UB = cen[s][f'ysult{b}_{q}'][j-2]
                        # stage[j]['yp'][b,q].LB = cen[s][f'ysult{b}_{q}'][j-2]
                ...
                for h in hidros.index: 
                    ...
                    stage[j]['ctr'][h].RHS = res[res['Iteradores']==t][f"{cen[s]['y'][inp]+1}"][hidros.COMPATIBILIDADE_SPT.loc[h]]
            inp+=1
            stage[j][f'm{j}'].update()
            ...
            # stage[j]['m{0}'.format(j)].write(f'SDDP_{j}{s}.lp')
            # stage[j]['m{0}'.format(j)].write(f'SDDP_{j}{s}.sol')
